...
// Render Controller: Render index.html with sleeplogs using EJS
const renderSleeplogs = async (req, res) => {
  const user_id = req.user._id
  try {
    const sleeplogs = await Sleeplog.find({user_id}).sort({createdAt: -1});
    res.render("index", { sleeplogs }); // Render index.ejs with sleeplogs data
  } catch (error) {
...
const renderSleeplog = async (req, res) => {
  try {
    const { id } = req.params;
    const user_id = req.user._id;
    const sleeplog = await Sleeplog.findById(id).where('user_id').equals(user_id);
    if (!sleeplog) {
...
const addSleeplog = async (req, res) => {
  try {
    const { date, hoursSelpt, quality, comments } = req.body;
    const newSleeplog = new Sleeplog({ date, hoursSelpt, quality, comments });
...
const deleteSleeplog = async (req, res) => {
  try {
    const { id } = req.params;
    const user_id = req.user._id;
    const sleeplog = await Sleeplog.findByIdAndDelete({ _id: id,user_id: user_id });
    if (!sleeplog) {
...
const updateSleeplog = async (req, res) => {
  try {
    ...
    const { date, hoursSelpt, quality, comments } = req.body;
    const updatedSleeplogData = {date, hoursSelpt, quality, comments };

    // Update the sleeplog with the new data
    const updatedSleeplog = await Sleeplog.findOneAndUpdate({ _id: id, user_id: user_id }, updatedSleeplogData, { new: true });
